...
public class Main {
	static int n = 10; // number of processes
	static int m = n; // number of etapas
	static int TOTAL_THREADS = 2*n;
	static int TOTAL_STEPS = 2*m;
	...
	public static void main(String[] args) throws InterruptedException {
		...
		for(int i = 0; i < TOTAL_THREADS; i++){
			if(i % 2 == 0)
				threads_list[i] = new Thread(new ThreadExercise2_increment(i)); //inner_loops, i));
			else
				threads_list[i] = new Thread(new ThreadExercise2_decrement(i)); //inner_loops, i));
